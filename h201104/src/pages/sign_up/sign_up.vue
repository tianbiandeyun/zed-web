    ...
    async mounted() {
      ...
      await this.$store.dispatch("fetch", {
        ...
        if (res.result === "failure") {
          ...
          this.info = res.back_value[0];
        }
      });

      // 如果报名过则显示报名信息
      if (!this.is_registration.back_value) {
        await this.$store.dispatch("fetch", {
          im: this.$Config.INTER_FACE.get_registration_info,
          fps: {
          ...
          if (res.result === "failure") {
            this.$Utils.closeWaiting();
            this.$Utils.showErrorInfo(res, "get_registration_info");
          } else {
